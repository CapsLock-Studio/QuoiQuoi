class Admin::TravelPhotosController < AdminController
  ...
  def index
    @travel_photos = ArticleImage.where(article_id: params[:article_id])
  ...
  def create
    @travel_photo = ArticleImage.new(travel_photo_params.merge({article_id: params[:article_id]}))
  ...
  def set_travel_photo
    @travel_photo = ArticleImage.find(params[:id])
  end
end
